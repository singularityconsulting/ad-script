                    ...
                    if (amazonSlotObj.sizes && amazonSlotObj.sizes.length > 0) {
                        ...
                        apstag.fetchBids({slots: [amazonSlotObj]}, function (bids) {
                            ...
                            googletag.cmd.push(function () {
                                ...
                                const slotName = curSlot.getAdUnitPath();
                                const slotInMap = slotsMap.find(s => s.slotName === slotName);
                                const slotElement = document.querySelector(`#${slotInMap.slotId}`);
                                if (slotElement) {

                                    const intersectionObserver = new IntersectionObserver(([entry]) => {
                                        if (entry.intersectionRatio <= 0 || entry.target.firstChild) return;
                                        console.log('Loading', slotName)
                                        googletag.pubads().refresh([curSlot])
                                    });
                                    intersectionObserver.observe(slotElement);
                                }
